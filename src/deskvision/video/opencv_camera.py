"""Shared OpenCV camera source for Windows and explicit macOS core testing."""
...
class OpenCVCameraSource:
    """Read one local camera through OpenCV.
    ...
    def _load_cv2(self) -> Any:
        ...
        try:
            ...
            raise CameraOpenError(
                "opencv-contrib-python is required to open the camera; "
    ...
    def _backend_flag(self, cv2: Any) -> int:
        ...
        if flag is None:
            raise CameraOpenError(
                f"OpenCV was built without the requested camera backend {backend!r}"
            )
        return int(flag)

    def configure(self, config: CameraConfig) -> None:
        """Update a stopped source without resetting the frame identity sequence.

        The runtime owns stopping/joining capture before calling this method;
        keeping the same source preserves frame IDs across a camera switch.
        """
        ...
        with self._lock:
            if self._capture is not None:
                raise CameraSourceError("close the camera before reconfiguring it")
            self.config = config
            self._last_error = None

    def _open_help(self) -> str:
        if self.config.backend.lower() == "avfoundation" or (
            self.config.backend.lower() == "any" and platform.system() == "Darwin"
        ):
            return (
                "; on macOS, allow Camera access for the launching app "
                "(Terminal/Codex) in System Settings > Privacy & Security > Camera, "
                "then retry. Also check the selected device and other camera apps"
            )
        return "; check the selected device, Windows camera privacy settings and other camera apps"
    ...
    def open(self) -> None:
        ...
        if not self._capture_is_open(capture):
            ...
            message = (
                ...
                f"with backend {self.config.backend}{self._open_help()}"
            )

...
SETTING_GROUPS = (
    SettingGroup(
        ...
        (
            _field(
                ...
                "port",
                "首选端口",
                "默认 9000；自动模式检索 9000–10000（含两端），跳过占用或禁用端口。"
    ...
    SettingGroup(
        "camera",
        "摄像头",
        "摄像头采集规格；即时切换设备请使用上方的摄像头选择面板。",
        (
            _field("camera", "device_index", "设备编号", "通常内置/首个摄像头为 0。", "integer", minimum=0, maximum=32, step=1),
            _field("camera", "backend", "采集后端", "使用与当前配置平台匹配的本地采集后端。", "select", options=("msmf", "dshow", "avfoundation", "any")),
...
def camera_backend_options(target_os: str) -> tuple[str, ...]:
    return ("avfoundation", "any") if target_os == "macos" else ("msmf", "dshow", "any")


def _serialize_field(field: SettingField, config: DeskVisionConfig) -> dict[str, object]:
    result = field.serialize(config)
    if (field.section, field.name) == ("camera", "backend"):
        result["options"] = list(camera_backend_options(config.deployment.target_os))
        result["description"] = (
            "macOS 使用 avfoundation；any 由 OpenCV 自动选择。"
            if config.deployment.target_os == "macos"
            else "Windows 优先 msmf；异常时尝试 dshow 或 any。"
        )
    return result
...
def _atomic_write_yaml(path: Path, payload: Mapping[str, object]) -> None:
    ...
    text = (
        ...
        "# Normal settings apply on restart; explicit camera selection may apply live.\n"
...
class RuntimeSettingsController:
    ...
    def settings_state(self) -> dict[str, object]:
        with self._lock:
            ...
            return {
                ...
                "target_os": configured.deployment.target_os,
                ...
                "groups": [
                    {
                        ...
                        "fields": [
                            _serialize_field(field, configured) for field in group.fields
                        ],
                    }
                    for group in SETTING_GROUPS
                ],
    ...
    def service_state(self) -> dict[str, object]:
        with self._lock:
            ...
            return {
                ...
                "mode": self.mode,
                "target_os": self.active_config.deployment.target_os,
    ...
    def _save_unlocked(self, payload: Mapping[str, object]) -> dict[str, object]:
        ...
        if candidate.camera.backend not in camera_backend_options(candidate.deployment.target_os):
            raise ValueError(
                f"camera backend {candidate.camera.backend!r} is not supported by "
                f"target_os {candidate.deployment.target_os!r}"
            )
